...
hourLocal <- t$hour
rt <- range(t)
days <- round(as.numeric(difftime(rt[2],rt[1], units="days")))
...
par(mar=c(1,1,1,1))
cm <- colormap(t$yday, col=oceColorsTurbo)
drawPalette(colormap=cm)
o <- sample(length(t$yday))
plot(x[o],y[o],asp=1,pch=20,cex=1/3,axes=FALSE, xlab="", ylab="", col=cm$zcol[o])
